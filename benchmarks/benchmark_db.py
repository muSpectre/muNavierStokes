...
driver reads rows back from this CSV (by default the most recent run for the
benchmark in question) and renders tables and plots from them — so a page can be
regenerated at any time without re-running anything, and old runs stay
reproducible.

This mirrors µGrid's ``examples/benchmark_db.py``; the schema is shared so the
two projects' benchmark databases look the same.
...
Identity of the data point:
  ...
  label      series within a study, e.g. a config key ("cpu1"/"cpuN"/"gpu1"/
             "gpuN") or a rank count
Run parameters:
  device nranks dim n npts steps
Results:
  secs mpoints gbps
...
DB_PATH = os.path.join(HERE, "results.csv")
...
# --------------------------------------------------------------------------- #
# Device / MPI configuration vocabulary (shared by the device-comparison plot)
# --------------------------------------------------------------------------- #
...
def plan_configs(ncores, nb_gpus, want_gpu):
    ...
    if want_gpu and nb_gpus >= 1:
        cfgs.append(("gpu1", "cuda", 1))
    if want_gpu and nb_gpus > 1:
        cfgs.append(("gpuN", "cuda", nb_gpus))
    return cfgs
...
def load(path=DB_PATH):
    ...
    if not os.path.exists(path):
        return []
    numeric = {"dirty", "nranks", "dim", "n", "npts", "steps",
               "secs", "mpoints", "gbps"}
    rows = []
