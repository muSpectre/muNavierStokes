...
# I/O parameters
nb_steps = 100000
screen_interval = 1  # output to screen every `screen_interval` steps
...
ns = NavierStokes(nb_grid_pts, physical_size, viscosity, communicator=MPI.COMM_WORLD)
...
# uarr_cqks = np.zeros((3,) + fft.nb_fourier_grid_pts, dtype=complex)
...
# fac_qks = np.zeros_like(wavevector_sq_qks)
# fac_qks[np.logical_not(zero_wavevector_qks)] = velocity_amplitude * wavevector_sq_qks[np.logical_not(zero_wavevector_qks)] ** (-5 / 6)
# uarr_cqks *= fac_qks
# # Project
# uarr_cqks = to_incompressible(uarr_cqks)
